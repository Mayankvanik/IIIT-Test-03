...
Graph nodes:
  router       → classify user intent into one of 4 routes
  retrieve     → hybrid Qdrant search (single or multi-contract)
  extract      → clause extraction for one or all 41 categories
  compare_risk → cross-contract diff + risk flagging
  answer       → final answer generation with citations
  grade        → hallucination + relevance grader (loops back if low)
  respond      → format and stream final response

Run:
  python chatbot.py                  # interactive CLI
  from chatbot import graph, State   # import into Streamlit / FastAPI
"""
...
from retrieval import (
    fetch_all_chunks,
    list_contracts,
    retrieve_cross_contract,
    retrieve_for_contract,
)
...
def chat():
    print("=" * 60)
    print("  CUAD Contract Analysis Chatbot")
